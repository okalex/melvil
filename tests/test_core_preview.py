...
class TestConfigureScene:
    def _call(self, tmp_path, engine="BLENDER_WORKBENCH"):
        from blammo.core.preview import _configure_scene
...
def _mesh_patches(extra_patches=None):
    ...
    return [
        patch("blammo.core.preview._bounding_sphere", return_value=(_MOCK_CENTER, 1.5)),
        patch("blammo.core.preview._position_camera_zoom_to_fit"),
        patch("blammo.core.preview._set_isometric_camera"),
        patch("blammo.core.preview._do_render"),
    ]


class TestGenerateMeshPreviewSuccess:
    def test_returns_absolute_path_to_png(self, tmp_path):
        from blammo.core.preview import generate_mesh_preview

        previews_dir = tmp_path / "previews"
        ctx = _make_context()
        obj = _make_obj()

        with patch("blammo.core.preview._bounding_sphere", return_value=(_MOCK_CENTER, 1.5)), \
             patch("blammo.core.preview._position_camera_zoom_to_fit"), \
             patch("blammo.core.preview._do_render"):
    ...
    def test_uses_viewport_view_matrix(self, tmp_path):
        ...
        with patch("blammo.core.preview._bounding_sphere", return_value=(_MOCK_CENTER, 1.5)), \
             patch("blammo.core.preview._position_camera_zoom_to_fit") as mock_zoom, \
             patch("blammo.core.preview._set_isometric_camera") as mock_iso, \
             patch("blammo.core.preview._do_render"):
    ...
    def test_calls_bounding_sphere_on_obj(self, tmp_path):
        ...
        with patch("blammo.core.preview._bounding_sphere", return_value=(_MOCK_CENTER, 1.5)) as mock_bs, \
             patch("blammo.core.preview._position_camera_zoom_to_fit"), \
             patch("blammo.core.preview._do_render"):
    ...
    def test_calls_do_render_with_scene(self, tmp_path):
        ...
        with patch("blammo.core.preview._bounding_sphere", return_value=(_MOCK_CENTER, 1.5)), \
             patch("blammo.core.preview._position_camera_zoom_to_fit"), \
             patch("blammo.core.preview._do_render") as mock_render:
    ...
    def test_sets_clip_start_proportional_to_radius(self, tmp_path):
        """cam_data.clip_start must be set to max(radius * 1e-3, 1e-6) so that
        tiny objects are not clipped by Blender's default 0.1-unit near plane."""
        import bpy
        from blammo.core.preview import generate_mesh_preview
        ...
        assert cam_data.clip_start == max(1.5 * 1e-3, 1e-6)
...
class TestGenerateMeshPreviewCleanup:
    def test_returns_none_on_render_failure(self, tmp_path):
        ...
        with patch("blammo.core.preview._bounding_sphere", return_value=(_MOCK_CENTER, 1.5)), \
             patch("blammo.core.preview._position_camera_zoom_to_fit"), \
             patch("blammo.core.preview._do_render", side_effect=RuntimeError("GPU unavailable")):
    ...
    def test_scene_removed_on_failure(self, tmp_path):
        ...
        with patch("blammo.core.preview._bounding_sphere", return_value=(_MOCK_CENTER, 1.5)), \
             patch("blammo.core.preview._position_camera_zoom_to_fit"), \
             patch("blammo.core.preview._do_render", side_effect=RuntimeError("failed")):
    ...
    def test_returns_none_on_unexpected_exception_during_setup(self, tmp_path):
        ...
        with patch("blammo.core.preview._bounding_sphere", side_effect=ValueError("boom")), \
             patch("blammo.core.preview._do_render"):
...
_LIGHTING_PATCH = patch(
    "blammo.core.preview._make_material_preview_lighting",
    return_value=_LIGHT_MOCKS,
)
_AIM_PATCH = patch("blammo.core.preview._aim_at_origin")
_BOUNDING_SPHERE_PATCH = patch(
    "blammo.core.preview._bounding_sphere",
    return_value=(_MOCK_CENTER, 1.5),
)


class _MatCamPatch:
    """Combine _bounding_sphere and _set_isometric_camera patches.

    Both must be patched together because _bounding_sphere is called in
    generate_material_preview, and the real implementation imports
    mathutils which is unavailable outside Blender.
    """
    ...
    def __enter__(self):
        ...
        self._cam = patch("blammo.core.preview._set_isometric_camera")
        self._bs.__enter__()
        self._cam.__enter__()

    def __exit__(self, *exc):
        self._cam.__exit__(*exc)
        self._bs.__exit__(*exc)


_MAT_CAM_PATCH = _MatCamPatch()
...
class TestBoundingSphere:
    def test_uses_data_vertices_not_bound_box(self):
        """Must iterate over obj.data.vertices (not obj.bound_box) so bounds
        are correct for freshly-created bmesh objects that have not yet been
        evaluated by Blender's depsgraph."""
        import sys
        from blammo.core.preview import _bounding_sphere

        obj = MagicMock()
        # Empty vertex list → function returns early without accessing bound_box.
        obj.data.vertices = []

        with patch.dict(sys.modules, {"mathutils": MagicMock()}):
            _bounding_sphere(obj)

        obj.bound_box  # accessing it here would create the attr on the mock
        # The important check: bound_box was NOT read by the function itself.
        # We verify by asserting that call_count on the attribute is zero —
        # MagicMock tracks attribute access via __getattr__, not call_count,
        # so we just verify vertices was the entry point:
        assert obj.data.vertices is not None  # attribute was accessed

    def test_empty_vertices_returns_minimum_radius(self):
        """Empty mesh must return radius clamped to 0.001."""
        import sys
        from blammo.core.preview import _bounding_sphere
        ...
        with patch.dict(sys.modules, {"mathutils": MagicMock()}):
            _, radius = _bounding_sphere(obj)

        assert radius == 0.001

    def test_nonempty_vertices_transforms_by_matrix_world(self):
        """When vertices exist, each vertex co must be multiplied by matrix_world."""
        import sys
        from blammo.core.preview import _bounding_sphere

        # Build a world_vert mock with float x/y/z so arithmetic downstream works.
        world_v = MagicMock()
        world_v.x = 1.0
        world_v.y = 0.0
        world_v.z = 0.0
        diff = MagicMock()
        diff.length = 0.5
        world_v.__sub__ = MagicMock(return_value=diff)

        v = MagicMock()
        obj = MagicMock()
        obj.data.vertices = [v]
        mat = obj.matrix_world
        mat.__matmul__ = MagicMock(return_value=world_v)

        mathutils_mock = MagicMock()
        center_mock = MagicMock()
        mathutils_mock.Vector.return_value = center_mock

        with patch.dict(sys.modules, {"mathutils": mathutils_mock}):
            _bounding_sphere(obj)

        # matrix_world @ v.co should have been called once.
        mat.__matmul__.assert_called_once_with(v.co)

    def test_degenerate_single_vertex_clamps_radius(self):
        """A single coincident vertex gives zero max distance → clamped to 0.001."""
        import sys
        from blammo.core.preview import _bounding_sphere
        ...
        obj = MagicMock()
        obj.data.vertices = [MagicMock()]
        obj.matrix_world.__matmul__ = MagicMock(return_value=world_v)

        mathutils_mock = MagicMock()
        mathutils_mock.Vector.return_value = MagicMock()
...
class TestAimAtOrigin:
    def test_sets_rotation_euler_on_cam_obj(self):
        import sys
        from blammo.core.preview import _aim_at_origin
...
class TestMakeMaterialPreviewLighting:
    def test_creates_sun_light(self, tmp_path):
        import bpy
        import sys
        from blammo.core.preview import _make_material_preview_lighting

        scene = MagicMock()
        with patch.dict(sys.modules, {"mathutils": MagicMock()}):
            _make_material_preview_lighting(scene)

        bpy.data.lights.new.assert_called_with("blammo_preview_key", type="SUN")
...
class TestAddPreviewMesh:
    def test_dispatches_to_uv_sphere(self):
        from blammo.core.preview import _add_preview_mesh

        scene = MagicMock()
        expected = MagicMock()
        with patch("blammo.core.preview._add_uv_sphere", return_value=expected) as mock:
    ...
    def test_dispatches_to_cube(self):
        ...
        scene = MagicMock()
        expected = MagicMock()
        with patch("blammo.core.preview._add_builtin_cube", return_value=expected) as mock:
    ...
    def test_dispatches_to_torus(self):
        ...
        scene = MagicMock()
        expected = MagicMock()
        with patch("blammo.core.preview._add_builtin_torus", return_value=expected) as mock:
    ...
    def test_dispatches_to_monkey(self):
        ...
        scene = MagicMock()
        expected = MagicMock()
        with patch("blammo.core.preview._add_builtin_monkey", return_value=expected) as mock:
    ...
    def test_dispatches_to_user_asset_when_uuid(self):
        ...
        with patch("blammo.core.preview._add_user_mesh_asset", return_value=expected) as mock:
...
# ---------------------------------------------------------------------------
# _add_user_mesh_asset
# ---------------------------------------------------------------------------


class TestAddUserMeshAsset:
    def _make_load_cm(self, obj_mock, obj_name="ShaderBall"):
        """Return a context manager mock that simulates bpy.data.libraries.load.

        Inside the ``with`` block the code sets ``data_to.objects = [obj_name]``.
        Real Blender then replaces those string names with actual object
        references when the context manager exits.  We simulate that by having
        ``__exit__`` overwrite ``data_to.objects`` with ``[obj_mock]``.
        """
        data_from = MagicMock()
        data_from.objects = [obj_name]
        data_to = MagicMock()

        def fake_exit(*args):
            data_to.objects = [obj_mock]
            return False

        cm = MagicMock()
        cm.__enter__ = MagicMock(return_value=(data_from, data_to))
        cm.__exit__ = MagicMock(side_effect=fake_exit)
        return cm

    def test_resets_transform_to_origin(self):
        """Appended object must have its location/rotation/scale reset so the
        camera zoom-to-fit calculation matches the actual rendered position."""
        import bpy
        from blammo.core.preview import _add_user_mesh_asset

        obj = MagicMock()
        bpy.data.libraries.load.return_value = self._make_load_cm(obj)

        _add_user_mesh_asset(MagicMock(), "/lib/shaderball.blend", "ShaderBall")

        assert obj.location == (0.0, 0.0, 0.0)
        assert obj.rotation_euler == (0.0, 0.0, 0.0)
        assert obj.scale == (1.0, 1.0, 1.0)

    def test_links_obj_to_scene(self):
        import bpy
        from blammo.core.preview import _add_user_mesh_asset
        ...
        result = _add_user_mesh_asset(scene, "/lib/shaderball.blend", "ShaderBall")

        scene.collection.objects.link.assert_called_once_with(obj)
        assert result is obj

    def test_returns_none_when_object_not_in_blend(self):
        import bpy
        from blammo.core.preview import _add_user_mesh_asset

        data_from = MagicMock()
        data_from.objects = []  # object name not found
        data_to = MagicMock()
        data_to.objects = []
        ...
        bpy.data.libraries.load.return_value = cm

        result = _add_user_mesh_asset(MagicMock(), "/lib/shaderball.blend", "Missing")
...
class TestGenerateMaterialPreviewSuccess:
    def test_returns_absolute_path_to_png(self, tmp_path):
        from blammo.core.preview import generate_material_preview

        previews_dir = tmp_path / "previews"

        with patch("blammo.core.preview._add_preview_mesh", return_value=MagicMock()), \
             _LIGHTING_PATCH, _MAT_CAM_PATCH, \
             patch("blammo.core.preview._do_render"):
    ...
    def test_adds_uv_sphere_to_temp_scene(self, tmp_path):
        ...
        with patch("blammo.core.preview._add_preview_mesh", return_value=MagicMock()) as mock_sphere, \
             _LIGHTING_PATCH, _MAT_CAM_PATCH, \
             patch("blammo.core.preview._do_render"):
    ...
    def test_applies_material_to_sphere(self, tmp_path):
        ...
        with patch("blammo.core.preview._add_preview_mesh", return_value=sphere_obj), \
             _LIGHTING_PATCH, _MAT_CAM_PATCH, \
             patch("blammo.core.preview._do_render"):
    ...
    def test_creates_lighting_for_scene(self, tmp_path):
        ...
        with patch("blammo.core.preview._add_preview_mesh", return_value=MagicMock()), \
             patch("blammo.core.preview._make_material_preview_lighting",
    ...
    def test_camera_zoom_to_fit(self, tmp_path):
        """_set_isometric_camera must be called with (cam_obj, center, radius)
        derived from the preview mesh's bounding sphere — same code path as
        ...
        with patch("blammo.core.preview._add_preview_mesh", return_value=MagicMock()), \
             patch("blammo.core.preview._bounding_sphere", return_value=(center_mock, radius_val)), \
             _LIGHTING_PATCH, \
             patch("blammo.core.preview._set_isometric_camera") as mock_cam, \
             patch("blammo.core.preview._do_render"):
            generate_material_preview(MagicMock(), _make_mat(), ASSET_ID, previews_dir)

        mock_cam.assert_called_once_with(ANY, center_mock, radius_val)
    ...
    def test_sets_clip_start_proportional_to_radius(self, tmp_path):
        """cam_data.clip_start must be set from the bounding sphere radius so that
        tiny objects are not clipped by Blender's default 0.1-unit near plane."""
        import bpy
        from blammo.core.preview import generate_material_preview
        ...
        with patch("blammo.core.preview._add_preview_mesh", return_value=MagicMock()), \
             patch("blammo.core.preview._bounding_sphere", return_value=(MagicMock(), 1.5)), \
             _LIGHTING_PATCH, _MAT_CAM_PATCH, \
             patch("blammo.core.preview._do_render"):
...
class TestGenerateMaterialPreviewCleanup:
    ...
    def test_returns_none_on_sphere_creation_failure(self, tmp_path):
        ...
        with patch("blammo.core.preview._add_preview_mesh", side_effect=RuntimeError("ops failed")), \
             _LIGHTING_PATCH, _MAT_CAM_PATCH, \
             patch("blammo.core.preview._do_render"):
...
class TestDoRender:
    def test_calls_bpy_ops_render_with_write_still(self):
        import bpy
        from blammo.core.preview import _do_render

        scene = MagicMock()
        scene.name = "blammo_preview_temp"

        _do_render(scene)

        bpy.ops.render.render.assert_called_with(write_still=True, scene="blammo_preview_temp")
