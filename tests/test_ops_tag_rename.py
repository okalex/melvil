"""Tests for ops/tag_rename.py — MELVIL_OT_tag_rename."""
...
def _make_op(tag_id="", name=""):
    from melvil.ops.tag_rename import MELVIL_OT_tag_rename
    op = MELVIL_OT_tag_rename()
    op.tag_id = tag_id
    op.name = name
    return op
...
def test_bl_idname():
    from melvil.ops.tag_rename import MELVIL_OT_tag_rename
    assert MELVIL_OT_tag_rename.bl_idname == "melvil.tag_rename"


def test_poll_always_true():
    from melvil.ops.tag_rename import MELVIL_OT_tag_rename
    assert MELVIL_OT_tag_rename.poll(MagicMock()) is True
...
class TestInvoke:
    ...
    def test_missing_tag_returns_cancelled(self, conn):
        ...
        with patch("melvil.ops.tag_rename.resolve_db_path", return_value=":memory:"), \
             patch("melvil.ops.tag_rename.open_db", _mock_open_db(conn)):
            result = op.invoke(MagicMock(), MagicMock())
        assert result == {"CANCELLED"}
...
class TestExecute:
    ...
    def test_library_not_configured_returns_cancelled(self):
        ...
        op = _make_op(tag_id=TAG_ID, name="steel")
        with patch("melvil.ops.tag_rename.resolve_db_path", side_effect=LibraryNotConfiguredError("x")):
            result = op.execute(MagicMock())
        assert result == {"CANCELLED"}
