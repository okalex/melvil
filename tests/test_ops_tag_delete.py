"""Tests for ops/tag_delete.py — BLAMMO_OT_tag_delete."""
...
def _make_op(tag_id=""):
    from blammo.ops.tag_delete import BLAMMO_OT_tag_delete
    op = BLAMMO_OT_tag_delete()
    op.tag_id = tag_id
    op._tag_name = ""
    op._usage_count = 0
    return op
...
def test_bl_idname():
    from blammo.ops.tag_delete import BLAMMO_OT_tag_delete
    assert BLAMMO_OT_tag_delete.bl_idname == "blammo.tag_delete"


def test_poll_always_true():
    from blammo.ops.tag_delete import BLAMMO_OT_tag_delete
    assert BLAMMO_OT_tag_delete.poll(MagicMock()) is True
...
class TestInvoke:
    ...
    def test_missing_tag_returns_cancelled(self, conn):
        ...
        with patch("blammo.ops.tag_delete.resolve_db_path", return_value=":memory:"), \
             patch("blammo.ops.tag_delete.open_db", _mock_open_db(conn)):
            result = op.invoke(MagicMock(), MagicMock())
        assert result == {"CANCELLED"}
    ...
    def test_populates_name_and_usage(self, conn_with_asset):
        ...
        ctx = MagicMock()
        with patch("blammo.ops.tag_delete.resolve_db_path", return_value=":memory:"), \
             patch("blammo.ops.tag_delete.open_db", _mock_open_db(conn_with_asset)):
...
class TestExecute:
    ...
    def test_library_not_configured_returns_cancelled(self):
        ...
        op = _make_op(tag_id=TAG_ID)
        with patch("blammo.ops.tag_delete.resolve_db_path", side_effect=LibraryNotConfiguredError("x")):
            result = op.execute(MagicMock())
        assert result == {"CANCELLED"}
