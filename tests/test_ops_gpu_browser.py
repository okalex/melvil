"""Tests for ops/gpu_browser.py — MELVIL_OT_gpu_browser viewport browser."""
...
class TestMetadata:
    def test_bl_idname(self):
        from melvil.ops.gpu_browser import MELVIL_OT_gpu_browser

        assert MELVIL_OT_gpu_browser.bl_idname == "melvil.gpu_browser"

    def test_bl_label(self):
        from melvil.ops.gpu_browser import MELVIL_OT_gpu_browser

        assert MELVIL_OT_gpu_browser.bl_label == "Melvil Browser (GPU)"

    def test_bl_options_contains_internal(self):
        from melvil.ops.gpu_browser import MELVIL_OT_gpu_browser

        assert "INTERNAL" in MELVIL_OT_gpu_browser.bl_options
...
class TestPoll:
    def test_returns_true_in_view3d(self):
        ...
        assert MELVIL_OT_gpu_browser.poll(ctx) is True
    ...
    def test_returns_false_outside_view3d(self):
        ...
        assert MELVIL_OT_gpu_browser.poll(ctx) is False
