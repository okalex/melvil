"""Tests for ops/asset_set_kit.py — BLAMMO_OT_asset_set_kit."""
...
def _make_op(asset_id="", kit_id=""):
    from blammo.ops.asset_set_kit import BLAMMO_OT_asset_set_kit

    op = BLAMMO_OT_asset_set_kit()
...
def test_bl_idname():
    ...
    assert BLAMMO_OT_asset_set_kit.bl_idname == "blammo.asset_set_kit"


def test_bl_label():
    from blammo.ops.asset_set_kit import BLAMMO_OT_asset_set_kit

    assert BLAMMO_OT_asset_set_kit.bl_label == "Move to Kit"
...
def test_poll_always_true():
    ...
    assert BLAMMO_OT_asset_set_kit.poll(MagicMock()) is True
...
def test_execute_no_asset_id_returns_cancelled():
    ...
    with patch("blammo.ops.asset_set_kit.open_db"), \
         patch("blammo.ops.asset_set_kit.resolve_db_path"):
...
def test_execute_unknown_asset_returns_cancelled(conn):
    ...
    with patch("blammo.ops.asset_set_kit.open_db") as mock_open, \
         patch("blammo.ops.asset_set_kit.resolve_db_path"):
...
def test_execute_library_not_configured_returns_cancelled():
    ...
    with patch("blammo.ops.asset_set_kit.resolve_db_path",
