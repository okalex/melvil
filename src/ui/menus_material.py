"""
Blammo submenu for the material slot context menu (Properties ▸ Material
▸ option arrow).

``BLAMMO_MT_material_submenu`` appears as a "Blammo! ▶" entry in
...
class BLAMMO_MT_material_submenu(bpy.types.Menu):
    """Blammo material actions — shown as a sub-menu in the material context menu."""

    bl_idname = "BLAMMO_MT_material_submenu"
    bl_label = "Blammo!"
    ...
    def draw(self, context):
        ...
        if has_material:
            ...
            layout.separator()
            layout.operator("blammo.save_asset", text="Save as Asset", icon="EXPORT")
...
def _draw_material_entry(self, context):
    """Appended to MATERIAL_MT_context_menu to insert the Blammo sub-menu.
    ...
    if obj is None:
        return
    self.layout.menu(BLAMMO_MT_material_submenu.bl_idname)
...
def register() -> None:
    register_submenu(BLAMMO_MT_material_submenu, "MATERIAL_MT_context_menu", _draw_material_entry)


def unregister() -> None:
    unregister_submenu(BLAMMO_MT_material_submenu, "MATERIAL_MT_context_menu", _draw_material_entry)
